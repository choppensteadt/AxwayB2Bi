...
NSString * AxwayB2BCommunityManagement$ModuleRequireFormat = @"(function(exports){"
...
@implementation AxwayB2BCommunityManagementObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_axwayb2bcommunitymanagement);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_axwayb2bcommunitymanagement gc];
}
...
{
	// create AxwayB2BCommunityManagement global object
	NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];

	// Load the "AxwayB2BCommunityManagement" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_axwayb2bcommunitymanagement = [[AxwayB2BCommunityManagementObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_axwayb2bcommunitymanagement];

	NSString *_axwayb2bcommunitymanagementNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _axwayb2bcommunitymanagementNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_axwayb2bcommunitymanagement valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:AxwayB2BCommunityManagement$ModuleRequireFormat, dirname, filename, code];
...
- (TiModule *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
	...
	if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
		@throw [NSException exceptionWithName:@"org.axwayb2bcommunitymanagement.kroll"
