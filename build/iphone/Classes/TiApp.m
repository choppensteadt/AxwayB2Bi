...
- (NSString*)systemUserAgent
{
    ...
    NSString *kAxwayB2BCommunityManagementUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
    return [NSString stringWithFormat:@"%@/%s (%@)",kAxwayB2BCommunityManagementUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo];
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a AxwayB2BCommunityManagement app using the node.js CLI
