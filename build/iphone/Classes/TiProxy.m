...
//Rare exceptions to indicate a bug in the _axwayb2bcommunitymanagement code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_axwayb2bcommunitymanagement = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_axwayb2bcommunitymanagement]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.axwayb2bcommunitymanagement.module"
